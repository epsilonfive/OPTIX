...
void optix_SetDefaultColors(void) {
    ...
    optix_guicolors.bgcolor = 255;
    optix_guicolors.colora = 255;
    optix_guicolors.colorb = 0;
    optix_guicolors.textfgcolor = 0;
    optix_guicolors.textbgcolor = 255;
    optix_guicolors.ttextfgcolor = 255;
    optix_guicolors.ttextbgcolor = 0;
...
void optix_RenderWindow(const char title[], uint16_t width, uint8_t height) {
    ...
    gfx_SetColor(optix_guicolors.colorb);
    gfx_Rectangle(xprint, yprint + titlewidth - 1, width, height + 1);
    //the title bar
...
int optix_AddMenu(uint16_t x, uint8_t y, uint8_t xpadding, uint8_t ypadding, uint8_t width, uint8_t height, uint8_t xspacing, uint8_t yspacing,
    ...
    if ((bool) (optix_menu = realloc(optix_menu, ++optix_guidata.nummenus * sizeof(struct optix_menu_t)))) {
        ...
        m->currselection = 0;
        m->text = NULL;
        m->textxcentering = m->textycentering = m->sprxcentering = m->sprycentering = 1;
    ...
    for (height = 0; height < m->height; height++) {
        for (width = 0; width < m->width; width++) {
            ...
            if (m->icon[iteration] != NULL && m->icon != NULL) {
                sprx = (m->xspacing - m->icon[iteration]->width) / 2;
                spry = (m->yspacing - m->icon[iteration]->height) / 2;
            }
            ...
            if (m->icon[iteration] != NULL && m->icon != NULL) {
                //display the sprite
                gfx_Sprite(m->icon[iteration], m->x + width * m->xspacing + sprx * m->sprxcentering + m->sprxoffset, m->y + height * m->yspacing + spry * m->sprycentering + m->spryoffset);
            }
